...
const CHANGELOG = [
  ...
  { version: "0.09", date: "2026-02-18", changes: [
    "Exercise swap — tap the swap icon mid-workout to see alternatives by muscle group",
    "Active workout overwrite protection — warns before replacing an in-progress workout",
  ]},
  { version: "0.08", date: "2026-02-18", changes: [
    "Offline mode — app loads instantly from local cache, syncs when connected",
  ]},
  { version: "0.07", date: "2026-02-18", changes: [
    "Warmup & cooldown notes on every workout",
    "Planned workouts — plan specific dates from calendar",
    "Plan editor — tap to review/edit before starting",
    "Template editor — edit name, exercises, weights",
                        ...
                        {(ex.warmupSets || []).length > 0 && ex.warmupWeight > 0 && (
                          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 10, color: T.textMuted }}>
                              {ex.warmupWeight} lb × {(ex.warmupSets || []).filter(s => s.completed).map(s => s.reps).join(", ")} reps
                            </span>
                          </div>
                        )}
...
// Mount the app
const mountEl = document.getElementById('workout-root') || document.getElementById('root');
